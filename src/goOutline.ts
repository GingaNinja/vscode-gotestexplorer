...
import { makeMemoizedByteOffsetConverter } from './utils';
...
export class GoDocumentSymbolProvider  {
	...
	public provideDocumentSymbols( uri :vscode.Uri): Thenable<vscode.SymbolInformation[]> {
		let options = { fileName: uri.fsPath };
		return documentSymbols(options).then(async decls => {
